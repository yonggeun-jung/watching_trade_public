...
# File paths
PRED_PORT_PATH = "watching_trade/output_tables/02_02_xgb_predictions_wgt_ports.csv"
PRED_NOPORT_PATH = "watching_trade/output_tables/03_02_xgb_predictions_wgt_noports.csv"
...
tick_labels = [monthly.iloc[j]['year_month'][:4] for j in tick_positions]

ax.set_xticks(tick_positions)
ax.set_xticklabels(tick_labels)

ax.set_ylabel('trade weight (log)')
...
plt.tight_layout()
plt.savefig(f'{OUTPUT_DIR}/timeseries_agg_wgt.pdf', bbox_inches='tight')
plt.close()
...
ax.plot(
    ...
    label='residual % (w/ port features)'
)
ax.plot(
    range(len(monthly)), monthly['resid_pct_noport'],
    color='tab:orange', linewidth=2, linestyle='--',
    label='residual % (w/o port features)'
)
...
plt.tight_layout()
plt.savefig(f'{OUTPUT_DIR}/timeseries_resid_agg_wgt.pdf', bbox_inches='tight')
plt.close()
...
ax.set_xlabel('actual trade weight (log)')
ax.set_ylabel('predicted trade weight (log)')
ax.set_aspect('equal', adjustable='box')
ax.tick_params(axis='both', width=0)

plt.tight_layout()
plt.savefig(f'{OUTPUT_DIR}/pred_scatter_port_wgt.pdf', bbox_inches='tight')
...
plt.tight_layout()
plt.savefig(f'{OUTPUT_DIR}/pred_scatter_noport_wgt.pdf', bbox_inches='tight')
...
plt.tight_layout()
plt.savefig(f'{OUTPUT_DIR}/timeseries_by_size_wgt.pdf', bbox_inches='tight')
